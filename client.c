#include<sys/socket.h>
#include<stdio.h>
#include<string.h>
#include<netdb.h>
#include<stdlib.h>
#include "../miracl.h"
#include<time.h>

#define BUFFER_SIZE 165
#define PORT 8090
#define NP 2          
#define PRIME_BITS 256

static big pd, pl, ph;
int sock_desc;
char buffer[BUFFER_SIZE];
char hash[32];
miracl *mip;

big readFile(char *fileName);
void writeFile(FILE *fptr, char *buffer,char *fileName);
char* hash_256();
void genkey();
void gen_prime(big p, int n);
void printHex(char *stng);
void encrypt(char *message);

int main() {
    FILE * keyFile;
    FILE * sFile;
    char message[BUFFER_SIZE];
    //init client setup
    int k;
    struct sockaddr_in client;
    memset(&client,0,sizeof(client));
    sock_desc=socket(AF_INET,SOCK_STREAM,0);

    if(sock_desc==-1) {
        printf("Error in socket creation");
        exit(1);
    }

    client.sin_family=AF_INET;
    client.sin_addr.s_addr=INADDR_ANY;
    client.sin_port=PORT;

    k=connect(sock_desc,(struct sockaddr*)&client,sizeof(client));
    if(k==-1) {
        printf("Error in connecting to server\n");
        exit(1);
    }
    //End client set-up

    //miracl set up
    big N, e, d, p, q, n, key, s;

    FILE *input_file;
    FILE *output_file;

    mip = mirsys(5000, 10);
   
    //Client - Server data exchange
    
    printf("Enter message that has to be sent to server:\n");
    fgets(buffer,BUFFER_SIZE,stdin); //get message
    strcpy(message,buffer);
    //hash_256();
    //printHex(digest);

    //Digest and concatinate
    strcpy(buffer,hash_256());
    strcat(buffer,";");
    strcat(buffer,message);
    genkey();

    printf("digest and message  %s\n",buffer);
    encrypt(buffer);

    
    //close(sock_desc);  
    return 0;
}

void writeFile(FILE *fptr, char *buffer,char *fileName) {
    fptr = fopen(fileName,"w");
    if(fptr==NULL) {
        printf("Unable to write file %s\n",fileName);
        exit(1);
    }
    fputs(buffer,fptr);
    fclose(fptr);
}

big readFile(char *fileName) {
    FILE *fptr;
    fptr = fopen(fileName,"r");
    char buffer[BUFFER_SIZE];
    if(fptr == NULL) {
        printf("ERROR reading file %s\n",fileName);
        exit(1);
    }
    fgets(buffer,BUFFER_SIZE,fptr);
    fclose(fptr);
    big n = mirvar(0);
    cinstr(n,buffer);
    //otnum(n,stdout);
    return n;
}



void gen_prime(big p, int n) {
    long seed1, seed2;
    seed1 = 1234567;
    seed2 = 8910112;

    int r, r1, r2;

    irand(seed1);
    bigbits(2 * n / 3, pd);
    nxprime(pd, pd);
    expb2(n - 1, ph);
    divide(ph, pd, ph);
    expb2(n - 2, pl);
    divide(pl, pd, pl);
    subtract(ph, pl, ph);

    irand(seed2);
    bigrand(ph, ph);
    add(ph, pl, ph);
    r1 = subdiv(pd, 12, pl);
    r2 = subdiv(ph, 12, pl);

    r = 0;
    while ((r1 * (r2 + r)) % 12 != 5)
        r++;

    incr(ph, r, ph);
    do
    {
        /* find p=2*r*pd+1 = 11 mod 12 */
        multiply(ph, pd, p);
        premult(p, 2, p);
        incr(p, 1, p);
        incr(ph, 12, ph);
    } while (!isprime(p));
}

void genkey() {

    /*  calculate public (N,e) and private keys (p,q)  *
    *  for rsa encryption                 */
    int i;
    long seed[2 * NP];

    big p[NP], N, a, b, n, e, d;

    big z, o, t;

    FILE *outfile;

    miracl *m = mirsys(5000, 10);

    gprime(15000); /* speeds up large prime generation */

    for (i = 0; i < NP; i++)
        p[i] = mirvar(0);

    N = mirvar(0);
    pd = mirvar(0);
    pl = mirvar(0);
    ph = mirvar(0);
    a = mirvar(0);
    b = mirvar(0);
    n = mirvar(0);
    e = mirvar(0);
    z = mirvar(0);
    o = mirvar(0);
    t = mirvar(0);
    d = mirvar(0);

    printf("Generating RSA parameters...\n");
    printf("Please wait....\n");
    convert(1, N); /* Converts 1 to big number and stores it inside ke*/
    for (i = 0; i < NP; i++)
    {
        printf("Generating prime number %d...\n", i + 1);
        gen_prime(p[i], PRIME_BITS);
        multiply(N, p[i], N);
    }

    /*n = (p-1)(q-1)*/
    decr(p[0], 1, a);
    decr(p[1], 1, b);
    multiply(a, b, n);

    /*Choose e s.t. 1 < e < n and e & n are co-prime*/
    convert(0, z);
    convert(1, o);
    do
    {
        bigrand(n, e);
        egcd(e, n, t);
    } while (mr_compare(e, z) == 0 || mr_compare(e, o) == 0 || mr_compare(t, o) != 0);

    /*Computing d s.t. ed = 1 mod n*/
    multi_inverse(1, &e, n, &d);

    m->IOBASE = 16;

    printf("\nKey Length = %d bits\n", logb2(N));

    outfile = fopen("public.key", "wt");
   // printf("Public Parameters:\n");

    printf("N = ");
    cotnum(N, stdout);
    cotnum(N, outfile);

    printf("e = ");
    cotnum(e, stdout);
    cotnum(e, outfile);

    fclose(outfile);

    outfile = fopen("private.key", "wt");
    printf("Private Parameters:\n");

    printf("p = ");
    cotnum(p[0], stdout);
    cotnum(p[0], outfile);

    printf("q = ");
    cotnum(p[1], stdout);
    cotnum(p[1], outfile);

    printf("d = ");
    cotnum(d, stdout);
    cotnum(d, outfile);

    printf("n = ");
    cotnum(n, stdout);
    cotnum(n, outfile);

    fclose(outfile);
}

char* hash_256() {
   
    int i;
    sha256 sh;
    shs256_init(&sh);
    for (i=0;buffer[i]!=0;i++) shs256_process(&sh,buffer[i]);
    shs256_hash(&sh,hash);    
    return hash;
}

void printHex(char *stng) {
    for (int i=0;i<32;i++) 
        printf("%02x",(unsigned char)stng[i]);
}

void encrypt(char *message){  
    big N,e,m,c;

    FILE *ifile;
    FILE *ofile;

    static char line[1024];
    static char buff[256];
    strcpy(line,message);
    int i,mlen,klen;


    N=mirvar(0);
    e=mirvar(0);
    m=mirvar(0);
    c=mirvar(0);

    ifile=fopen("public.key","rt");
    mip->IOBASE=16;
    cinnum(N,ifile);
    cinnum(e,ifile);
    fclose(ifile);
                                    
    klen = logb2(N)/7 - 1;    
    printf("Key length = %d\n",klen);

    
    cotnum(N,stdout);
    cotnum(e,stdout);

    
    ofile = fopen("output.enc","wt");
    printf("Enter the text to be encrypted: \n");
    mlen = strlen(line);
    printf("mlen %d\n",mlen);
    while(TRUE)
    {
        if(mlen < klen)
        {
            break;
        }
        for(int i=0; i<klen; i++)
            buff[i] = line[i];            
        buff[klen] = '\0';

        for(int i=klen; i<=mlen; i++)
            line[i-klen] = line[i];
        mlen = strlen(line);

        printf("Message = %s\n",line);
        mip->IOBASE=128;
        cinstr(m,buff);
        /*cotnum(m,stdout);*/
        powmod(m,e,N,c);
        mip->IOBASE=16;
        /*cotnum(c,stdout);*/
        cotnum(c,ofile);
    }

    /*Unpadded*/
    if (mlen < klen && mlen != 0)
    {
        for (int i = 0; i < mlen; ++i)
            buff[i] = line[i];
        buff[mlen] = '\0';

        mip->IOBASE=128;
        cinstr(m,buff);
        powmod(m,e,N,c);
        mip->IOBASE=16;
        /*cotnum(c,stdout);*/
        cotnum(c,ofile);
    }
    fclose(ofile);
}

